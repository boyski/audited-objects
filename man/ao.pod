...
will run marginally slower due to the overhead of auditing.  When it
comes to the second build, B<AO> will discover that the inputs are
unchanged and will only pretend to run the compiler, copying in the
...
full command name is I<foobar> but it can be abbreviated as I<foo>.

The set of post-mortem commands is extensible. A full list of currently
supported commands can be obtained from the server via C<ao help>, and
detailed help for a specific command is generally availble with C<ao
...
The list of targets and their prerequisites are printed in a format
suitable for inclusion by a makefile (see below).

=item get I<object> ...

Retrieves the specified derived I<object>s from the server.
...
Generates full dependency information in Makefile format. This can be
included in future builds.

=item -F|--make-file-full

Generates a single full, non-recursive, working Makefile with full
dependency information. The Makefile may require GNU make features.
...
The command line flags described above are actually just an alternate way
...
command-line flags map directly to a property, though not all properties
...
specifies a regular expression which would match all filenames
ending with ".c".
...
This also shows the set of files from which properties are being read.
...
default reasonably; you must set a Server in order to find a
server.>
...
which need not be reported. This is cancellation; each audit is subjected to cancellation
before its results are reported.
...
auditing substantially by tweaking the REs for your specific needs. Follwing is an
overview:
...
outside it are called "non-members". All member files are given a
I<project relative pathname> or PRP. Non-members are always referred
...
Project.Name defaults to be the basename of the Base.Dir value). Note that
...
For example, imagine building Perl in the Unix directory
/scratch/src/perl-5.8.8.  You could work without a project config at all,
...
on only that part for a while? Without a fixed base dir, I<ao> would now
...
the boundary explicitly. This will lock in the values above for any
working directory within the project tree.

However, perhaps a colleague is also working on perl-5.8.8 but chose to
extract the source to a directory with a different basename,  say
"/var/tmp/perl". Or maybe someone is working on perl-5.8.9 and wants to
be able to share the parts that haven't changed. Or you simply don't feel
that a version number should be part of the project name. So you may
prefer to add the following to the project properties file:
...
including the project properties file, will most commonly be under source control
...
are I<deterministic>, i.e. that if the same command is run
repeatedly in the same directory with the same file inputs, it will
produce the same file outputs.  While it is certainly possible to
...
avoided.  However, AO contains special-case code to ignore embedded
datestamps in certain common file formats such as archive files on Unix
and PE/COFF (.exe, .dll, .lib, and .obj) files on Windows.
...
Copyright 2002-2010 David Boyce. All rights reserved.
